...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    await logEvent({
      actorUserId,
      message: `Resource_Created: Name: ${resourceName}, price: ${Number(resourcePrice)}, unit: ${resourcePriceUnit}`,
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      ...
      return res.status(409).json({
        ...
        details: "A resource with the same name already exists. 💡 Please try a different name or check the existing list.",
      });
    }
