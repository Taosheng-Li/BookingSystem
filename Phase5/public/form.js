...
function buildValidationMessage(errors) {
  if (!Array.isArray(errors) || errors.length === 0) {
    return "Oops! Some inputs are invalid. Please check the fields and try again.";
  }
  ...
  return `Oops! Some inputs are invalid. Please fix the following fields to continue:\n\n${lines.join("\n")}`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        const msg =
          body?.details ||
          "A resource with the same name already exists. 💡Please try a different name or check the existing list.";
    ...
    const msgLines = ["✅ Resource added successfully!",
  `Confirmed: "${body?.data?.name ?? ""}" is now in the catalog.`,
  `ID: ${body?.data?.id ?? ""}`];
